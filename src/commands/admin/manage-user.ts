    ...
    collector.on("collect", async (buttonInteraction) => {
      if (buttonInteraction.customId.startsWith("manage-user-adjust-points")) {
        ...
        const modal = new ModalBuilder()
          .setCustomId(`new-points:${buttonInteraction.id}`)
        ...
        const modalInteraction = await buttonInteraction
          .awaitModalSubmit({ time: hide ? 390_000 : 8.64e7, filter: (i) => i.customId.endsWith(buttonInteraction.id) })
          .catch(() => undefined);
        ...
        const modal = new ModalBuilder()
          .setCustomId(`new-username:${buttonInteraction.id}`)
          .setTitle("Manage user")
          .addComponents(
            new ActionRowBuilder<TextInputBuilder>().addComponents(
              new TextInputBuilder()
